...
<a name="saveAuthorization"></a>`saveAuthorization` | `SAVE_AUTHORIZATION` | `Boolean=false`. If set to `true`, enables to save the value entered in authorize on localStorage of the browser. The key in local storage is written with the prefix `swagger-` complemented for "Name" currently used.
