...
import { saveAuthorizationToStorage } from "../../../helpers/save-authorization"
...
export default {
  ...
  [AUTHORIZE]: (state, { payload } ) =>{
    ...
    securities.entrySeq().forEach( ([ key, security ]) => {
      ...
      saveAuthorizationToStorage(configs, key, authorizationValue)
    })
  ...
  [LOGOUT]: (state, { payload } ) =>{
    ...
    let result = state.get("authorized").withMutations((authorized) => {
        auths.forEach((auth) => {
          ...
          saveAuthorizationToStorage(configs, auth, "")
        })
      })
